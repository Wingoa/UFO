...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 <xsl:output method="xhtml" encoding="utf-8" doctype-system="about:legacy-compat"
  ...
  <!--2015-10-18 ebb: As usual we altered the stylesheet template to read from the TEI namespace and output in XHTML. And we need an xsl:output statement, too.-->
    ...
    <xsl:variable name="dickinsonColl" select="collection('../Reports_XML/?select=*.xml')"/> 
    ...
    <xsl:template match="/">
      <html>
        ...
        <ul><xsl:apply-templates select="$dickinsonColl//body" mode="toc"/></ul>
              <hr/> <!--WHC: this means "horizontal rule" and it draws a line. -->
   ...
   <!--WHC: <div id="main"> isn't especially necessary here. It doesn't output anything visible, though screen-readers may use it. Since 2015 HTML 5 has preferred use of <main>, though <main> can only be used once per HTML document and <div type-"main"> can probably be used more than once if desired. See http://html5doctor.com/the-main-element/ . Anyway, neither of them is really needed here to make the XSLT work to output functional HTML. You could comment it out and it would work the same.-->
          </body>
